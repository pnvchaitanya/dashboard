...
def display_table(df: pd.DataFrame, table_name: str):
    ...
    # Hide ID columns
    df = df.drop(columns=["company_id", "round_id", "created_at", "updated_at", "created_at_dt"], errors="ignore").copy()
...
total_rounds = filtered["round_id"].nunique()
...
with tab3:
    ...
    company_metrics = (
        filtered.groupby(
            ["company_id", "company_name", "category_group", "status", "website", "linkedin_url"],
            dropna=False,
        )
...
allowed_round_ids = filtered["round_id"].dropna().unique()
allowed_company_ids = filtered["company_id"].dropna().unique()
